...
**What is DShop.Services.Products?**
----------------

DShop.Services.Products is a microservice designed for the products bounded context [DShop](https://github.com/devmentors/DNC-DShop) solution.

|Branch             |Build status                                                  
|-------------------|-----------------------------------------------------
|master             |[![master branch build status](https://api.travis-ci.org/devmentors/DNC-DShop.Services.Products.svg?branch=master)](https://travis-ci.org/devmentors/DNC-DShop.Services.Products)
|develop            |[![develop branch build status](https://api.travis-ci.org/devmentors/DNC-DShop.Services.Products.svg?branch=develop)](https://travis-ci.org/devmentors/DNC-DShop.Services.Products/branches)


**How to start the application?**
----------------

Service can be started locally via `dotnet run` (executed in the `/src/DShop.Services.Products` directory) or `./scripts/dotnet-run.sh` shell script, by default it will be available under http://localhost:5000.

You can also run the application using [Docker](https://www.docker.com) `docker run --name products-service -p 5000:5000 --network dshop-network devmentors/dshop.services.products` (include `-d` to run the container in the background).
